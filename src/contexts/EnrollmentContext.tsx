...
function enrollmentReducer(state: EnrollmentState, action: EnrollmentAction): EnrollmentState {
  switch (action.type) {
    ...
    // ✅ NYTT: Terms acceptance
    case "SET_TERMS_ACCEPTED":
      return {
        ...state,
        enrollmentData: {
          ...state.enrollmentData,
          termsAccepted: action.payload.accepted,
          termsAcceptedAt: action.payload.acceptedAt,
        },
      }
    ...
    case "GO_TO_NEXT_STEP": {
      ...
      const nextStep =
        currentIndex < stepOrder.length - 1 ? stepOrder[currentIndex + 1] : state.currentStep
    ...
    case "GO_TO_PREVIOUS_STEP": {
      const stepOrder: EnrollmentStep[] = ["contact", "courses", "summary", "confirmation"]
      const currentIndex = stepOrder.indexOf(state.currentStep)
      const prevStep = currentIndex > 0 ? stepOrder[currentIndex - 1] : state.currentStep
...
export function EnrollmentProvider({ children }: EnrollmentProviderProps) {
  ...
  const currentPricing = useMemo(() => {
    ...
    console.log("🧮 Beregner priser (useMemo):", {
      ...
      hasSiblings: state.enrollmentData.hasSiblings,
      siblingsCount: state.enrollmentData.siblings.length,
      termsAccepted: state.enrollmentData.termsAccepted,
    })
    ...
    state.enrollmentData.hasSiblings,
    state.enrollmentData.siblings,
    state.enrollmentData.pricing,
    state.enrollmentData.termsAccepted,
  ])

  const isFormValid = useMemo(() => {
    const { student, guardian, selectedCourses, hasSiblings, siblings, termsAccepted } =
      state.enrollmentData

    const hasStudentInfo =
      !!student.firstName.trim() && !!student.lastName.trim() && !!student.birthDate
    ...
    const hasSiblingsValid =
      !hasSiblings ||
      siblings.every((sibling) => sibling.firstName.trim().length > 0 && sibling.lastName.trim().length > 0)

    // ✅ NYTT: Krev terms
    return hasStudentInfo && hasGuardianInfo && hasCourses && hasPricing && hasSiblingsValid && termsAccepted
  ...
  const setTermsAccepted = useCallback((accepted: boolean) => {
    dispatch({
      ...
      payload: { accepted, acceptedAt: accepted ? new Date().toISOString() : null },
    })
  }, [])
  ...
  const validateCurrentStep = useCallback((): boolean => {
    ...
    switch (state.currentStep) {
      ...
      case "summary": {
        ...
        if (state.enrollmentData.hasSiblings) {
          const incompleteSiblings = state.enrollmentData.siblings.some(
        ...
        // ✅ NYTT: Block summary hvis terms ikke er akseptert
        if (!state.enrollmentData.termsAccepted) {
          errors.general = errors.general
            ? `${errors.general}. Du må bekrefte betingelser og vilkår`
            : "Du må bekrefte betingelser og vilkår"
          isValid = false
        }
  ...
  return <EnrollmentContext.Provider value={contextValue}>{children}</EnrollmentContext.Provider>
}
