...
export const useDanceClasses = (): UseDanceClassesReturn => {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        console.log('✅ Dance Classes SUCCESS:', classesResponse.documents.length, 'documents');
        console.log('   Sample class:', classesResponse.documents[0]?.name || 'No classes found');
        
        setDanceClasses(classesResponse.documents as DanceClass[]);
      } catch (classesError) {
        console.error('❌ Dance Classes FAILED:', classesError);
        throw new Error(`Failed to fetch dance classes: ${classesError.message}`);
      }
      ...
      try {
        ...
        console.log('✅ Schedules SUCCESS:', schedulesResponse.documents.length, 'documents');
        console.log('   Sample schedule:', schedulesResponse.documents[0] ? 
  ...
  const getSchedulesForClass = (classId: string): Schedule[] => {
    ...
    const filteredSchedules = schedules.filter(schedule => schedule.danceClassId === classId);
    console.log(`🔍 getSchedulesForClass: Found ${filteredSchedules.length} schedules for class ${classId}`);
  ...
  const getAvailableSpots = (schedule: Schedule): number => {
    ...
    const availableSpots = Math.max(0, schedule.maxStudents - schedule.currentStudents);
    console.log(`🔍 getAvailableSpots: ${availableSpots} spots available (${schedule.maxStudents} max - ${schedule.currentStudents} current)`);
  ...
  const hasAvailableSpots = (schedule: Schedule): boolean => {
    ...
    console.log(`🔍 hasAvailableSpots: ${hasSpots ? 'Yes' : 'No'} available spots`);
    return hasSpots;
  };
  ...
  const getClassesByType = (): Record<string, DanceClass[]> => {
    ...
    danceClasses.forEach(cls => {
      if (!cls.type) {
        console.log(`⚠️ getClassesByType: Class ${cls.name} has no type, skipping`);
        return;
      }
      
      if (!grouped[cls.type]) {
        grouped[cls.type] = [];
        console.log(`📂 getClassesByType: Created new group for type '${cls.type}'`);
      }
      grouped[cls.type].push(cls);
    });
    
    console.log('📊 getClassesByType: Grouped classes:', Object.keys(grouped).map(type => 
  ...
  const getClassesForAge = (age: number): DanceClass[] => {
    if (!age || age < 3 || age > 100) {
      console.log(`🔍 getClassesForAge: Invalid age ${age}, returning empty array`);
      return [];
    }
    
    const filteredClasses = danceClasses.filter(cls => {
      if (!cls.age) {
        console.log(`⚠️ getClassesForAge: Class ${cls.name} has no age requirement, excluding`);
        return false;
      }
      ...
      if (matches) {
        console.log(`✅ getClassesForAge: ${cls.name} (${ageRange}) matches age ${age}`);
      }
      ...
      const isAvailable = !cls.availableFromYear || cls.availableFromYear <= currentYear;
      
      if (!isAvailable) {
        console.log(`⏳ getClassesForAge: ${cls.name} not available until ${cls.availableFromYear}`);
      }
      
      return isAvailable;
    });
    
    console.log(`🎯 getClassesForAge: Found ${filteredClasses.length} classes for age ${age}:`, 
