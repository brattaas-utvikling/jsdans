...
const migrateCartItem = (item: any): CartItem => {
  ...
  console.log(`🔄 Migrating cart item: "${item.studentName}" → "${firstName} ${lastName}"`);
...
export const useCart = (packages: PricingPackage[] = []): UseCartReturn => {
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>(() => {
    try {
      ...
      if (saved) {
        ...
        console.log(`🔄 Migrated ${validMigratedItems.length} cart items to new format`);
        
        // Save migrated data back to localStorage
        if (validMigratedItems.length > 0) {
          localStorage.setItem(CART_STORAGE_KEY, JSON.stringify(validMigratedItems));
        }
  ...
  const detectFamily = useCallback((firstName: string, lastName: string): FamilyDetectionResult => {
    ...
    console.log('🔍 Existing cart items:', itemsWithHelpers.map(i => i.fullName));

    // Check if exact same person already exists
    const exactMatch = itemsWithHelpers.find(item => 
    ...
    const sameLastName = itemsWithHelpers.filter(item => 
      item.studentLastName.toLowerCase().trim() === trimmedLastName
    );
    ...
    const similarLastNames = itemsWithHelpers.filter(item => {
      ...
      const similarity = calculateNameSimilarity(existingLastName, trimmedLastName);
      return similarity > 0.8; // 80% similarity threshold
    });

    if (sameLastName.length > 0) {
      console.log('✅ Same last name found:', sameLastName.map(i => i.fullName));
      return {
        isLikelyFamily: true,
        confidence: 0.95,
    ...
    if (similarLastNames.length > 0) {
      console.log('🤔 Similar last name found:', similarLastNames[0].studentLastName);
      return {
        isLikelyFamily: true,
        confidence: 0.7,
  ...
  const determineHybridFamilyDiscount = (
    ...
    if (studentData.familyDiscountOverride !== undefined) {
      console.log('  - Manual override detected:', studentData.familyDiscountOverride);
    ...
    if (studentData.isSecondDancerInFamily !== undefined) {
      console.log('  - Explicit family setting:', studentData.isSecondDancerInFamily);
  ...
  const isDuplicateStudent = (firstName: string, lastName: string, excludeId?: string): boolean => {
    return cartItems.some(item => 
      item.id !== excludeId && 
      item.studentFirstName.toLowerCase().trim() === firstName.toLowerCase().trim() &&
      item.studentLastName.toLowerCase().trim() === lastName.toLowerCase().trim()
    );
  };

  // Add student to cart with HYBRID family discount logic
  const addToCart = useCallback(async (studentData: StudentData): Promise<string> => {
    try {
      console.log('🛒 Adding student to cart:', studentData.studentFirstName, studentData.studentLastName);
      ...
      if (!studentData.studentAge || studentData.studentAge < 3 || studentData.studentAge > 100) {
      ...
      // Check for duplicate names
      if (isDuplicateStudent(studentData.studentFirstName, studentData.studentLastName)) {
        throw new Error(`En student med navn "${studentData.studentFirstName} ${studentData.studentLastName}" finnes allerede i handlekurven`);
      }

      // HYBRID FAMILY DISCOUNT DETERMINATION 🎯
      const finalIsSecondDancer = determineHybridFamilyDiscount(studentData, cartItems.length);
      ...
      console.log(`✅ Added ${newItem.studentFirstName} ${newItem.studentLastName} to cart`);
      return newItem.id;
  ...
  // Update cart item
  const updateCartItem = useCallback((itemId: string, updates: Partial<CartItem>): void => {
    setCartItems(prev => prev.map(item => {
      if (item.id === itemId) {
        // Validate name changes for duplicates
        if ((updates.studentFirstName || updates.studentLastName) && 
            (updates.studentFirstName !== item.studentFirstName || updates.studentLastName !== item.studentLastName)) {
          const newFirstName = updates.studentFirstName || item.studentFirstName;
          const newLastName = updates.studentLastName || item.studentLastName;
          
          if (isDuplicateStudent(newFirstName, newLastName, itemId)) {
            throw new Error(`En student med navn "${newFirstName} ${newLastName}" finnes allerede`);
          }
        }
        ...
        return { 
          ...item, 
          ...updates,
          studentFirstName: updates.studentFirstName?.trim() || item.studentFirstName,
          studentLastName: updates.studentLastName?.trim() || item.studentLastName
        };
      }
      return item;
    }));
  ...
  const toggleFamilyDiscount = useCallback((itemId: string): void => {
    setCartItems(prev => prev.map(item => {
      if (item.id === itemId) {
        ...
        console.log(`🔄 Toggling family discount for ${item.studentFirstName}: ${item.isSecondDancerInFamily} → ${newValue}`);
        return { 
          ...item, 
  ...
  const duplicateStudent = useCallback((itemId: string): string => {
  ...
  const refreshCart = useCallback((): void => {
    ...
    setCartItems(prev => {
      ...
      if (validItems.length !== prev.length) {
        console.log(`🕒 Removed ${prev.length - validItems.length} expired items from cart`);
      }
  ...
  const getCartSummary = useCallback((): CartSummary => {
    ...
    const itemsWithHelpers = getCartItemsWithHelpers();
    const itemsWithPricing: CartItemWithPricing[] = itemsWithHelpers.map(item => {
    ...
    itemsWithPricing.forEach(item => {
      ...
      originalTotal += (item.pricing.originalPrice || item.pricing.total + item.pricing.discount);
    });
  ...
  const getTotalClassCount = useCallback((): number => {
    return cartItems.reduce((total, item) => total + item.selectedClasses.length, 0);
  }, [cartItems]);

  // Check if family discount applies
  const hasPotentialFamilyDiscount = useCallback((): boolean => {
    return cartItems.length > 1 && cartItems.some(item => item.isSecondDancerInFamily);
  }, [cartItems]);
  ...
  const getCartAnalytics = useCallback((): CartAnalytics => {
    ...
    return {
      ...
      totalClasses,
      averageClassesPerStudent: cartItems.length > 0 ? totalClasses / cartItems.length : 0,
      familyDiscountEligible: cartItems.filter(item => item.isSecondDancerInFamily).length,
  ...
  const validateCart = useCallback((): ValidationResult => {
    ...
    // Check for duplicate names
    const names = cartItems.map(item => `${item.studentFirstName} ${item.studentLastName}`.toLowerCase().trim());
    ...
    return {
      ...
      message: errors.length > 0 ? 'Feil i handlekurven' : 'Handlekurven er gyldig',
      errors
    };
  }, [cartItems]);
