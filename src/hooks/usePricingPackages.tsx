...
export const usePricingPackages = (): UsePricingPackagesReturn => {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        console.log('✅ Pricing Packages SUCCESS:', packagesResponse.documents.length, 'packages');
        ...
        if (packagesResponse.documents.length > 0) {
          ...
          packagesResponse.documents.forEach((pkg: any, index: number) => {
            console.log(`   ${index + 1}. "${pkg.name}" - ${pkg.priceInOre}øre (${pkg.type})`);
        ...
        throw new Error(`Failed to fetch pricing packages: ${packagesError.message}`);
      }
      ...
      setLoading(false);
      console.log('🏁 Pricing packages fetch operation completed (loading = false)');
    }
  };
  ...
  const getPackageByClassCount = (classCount: number): PricingPackage | null => {
    console.log(`🔍 getPackageByClassCount: Looking for package with ${classCount} classes`);
    console.log(`📋 Available packages:`, packages.map(p => `"${p.name}" (${p.type})`));
    ...
    if (foundPackage) {
      console.log(`✅ Found package: "${foundPackage.name}" - ${foundPackage.priceInOre}øre`);
  ...
  const getToddlerPackage = (): PricingPackage | null => {
    ...
    const toddlerPackage = packages.find(pkg => {
      const name = pkg.name.toLowerCase();
      return pkg.type === 'semester' && (
        name === 'toddler' ||
        name === 'toddler-pakke' ||
    ...
    if (toddlerPackage) {
      console.log(`✅ Found toddler package: "${toddlerPackage.name}" - ${toddlerPackage.priceInOre}øre`);
    } else {
  ...
  const getRegularPackages = (): PricingPackage[] => {
    ...
    console.log(`🔍 getRegularPackages: Found ${regularPackages.length} regular packages:`, 
  ...
  const getAllPackagesByType = (): Record<string, PricingPackage[]> => {
    ...
    console.log('📊 Grouped packages:', Object.keys(grouped).map(type => 
  ...
  const debugPackages = (): void => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      console.log(`   ${i} classes → ${result ? `"${result.name}"` : 'NOT FOUND'}`);
    }
  };
  ...
  useEffect(() => {
    ...
    if (packages.length > 0 && !loading) {
      console.log('📋 Final packages summary:', packages.map(p => 
        `"${p.name}" (${p.priceInOre}øre)`
      ).join(', '));
    }
