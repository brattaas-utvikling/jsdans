...
const validateFamilyDiscountSecurity = (
  ...
  if (isSecondDancerInFamily && selectedClasses.length === 1) {
    ...
    console.log(`🔒 Security check: Single class family discount attempted for ${className}`);
  ...
  if (isSecondDancerInFamily && selectedClasses.length > 1) {
    ...
    if (uniqueAgeGroups.size > 2) {
      console.log('🔒 Security check: Multiple age groups detected', Array.from(uniqueAgeGroups));
...
export const calculateSmartPackagePrice = (
  ...
  console.log('  - Classes:', selectedClasses.map(c => `${c.name} (${c.age})`));
  ...
  const securityCheck = validateFamilyDiscountSecurity(selectedClasses, isSecondDancerInFamily);
  ...
  if (isToddler) {
    ...
    const toddlerPackage = packages.find(p => 
      p.type === 'semester' && 
    ...
    console.log('✅ Using toddler package:', toddlerPackage.name, '-', toddlerPackage.priceInOre, 'øre');
    ...
    if (isSecondDancerInFamily) {
      ...
      console.log('  - ✅ Toddler family discount applied (30%):', familyDiscount, 'øre');
    }
  ...
  if (classCount === 1) {
    ...
    selectedPackage = packages.find(p => 
      p.type === 'semester' && p.name === '2 klasser'
    ) || null;
    ...
    selectedPackage = packages.find(p => 
      p.type === 'semester' && p.name === '3+ klasser'
    ) || null;
  ...
  console.log('✅ Found package:', selectedPackage.name, '-', selectedPackage.priceInOre, 'øre');
  ...
  if (isSecondDancerInFamily) {
    console.log('  - Family discount check: Is second dancer =', isSecondDancerInFamily);
    ...
    if (classCount === 1) {
      ...
      totalDiscount += familyDiscount;
      console.log('  - ✅ Family discount applied (15% for 1 class):', familyDiscount, 'øre');
      ...
      familyDiscount = Math.floor(total * 0.3);
      totalDiscount += familyDiscount;
      console.log('  - ✅ Family discount applied (30% for 2 classes):', familyDiscount, 'øre');
      
    } else if (classCount >= 3) {
      // Best rate 50% for 3+ classes
      familyDiscount = Math.floor(total * 0.5);
      totalDiscount += familyDiscount;
      console.log('  - ✅ Family discount applied (50% for 3+ classes):', familyDiscount, 'øre');
    }
  } else {
...
export const getPackageRecommendations = (
  ...
  if (classCount === 1) {
    ...
    const discount = packages.find(p => p.name === '2 klasser')?.discountAmount || 0;
    if (discount > 0) {
      recommendations.push(`Spar ${formatSmartPrice(discount)} med 2-klasser pakken!`);
    }
    recommendations.push('30% familierabatt tilgjengelig for danser nr. 2+');
  } else if (classCount >= 3) {
    const package3Plus = packages.find(p => p.name === '3+ klasser');
    if (package3Plus?.discountAmount) {
      recommendations.push(`Spar ${formatSmartPrice(package3Plus.discountAmount)} med 3+ klasser pakken!`);
    }
