...
export class SecurityLogger {
  ...
  static logRateLimitHit(identifier: string, attempts: number) {
    this.logSuspiciousActivity(`Rate limit exceeded: ${attempts} attempts`, identifier);
  }
}
...
export class RateLimiter {
  ...
  private static getStoredSubmissions(): Array<{ timestamp: number; identifier: string }> {
    try {
  ...
  private static setStoredSubmissions(submissions: Array<{ timestamp: number; identifier: string }>): void {
    try {
  ...
  static isAllowed(identifier: string): { allowed: boolean; reason?: string; waitTime?: number } {
    const now = Date.now();
...
export class BotDetection {
  ...
  static validateSubmission(formData: FormData, startTime: number): { isBot: boolean; reason?: string } {
    const honeypotValue = formData.get(this.HONEYPOT_FIELD);
    const fillTime = Date.now() - startTime;
    
    // Sjekk honeypot
    if (honeypotValue && String(honeypotValue).trim() !== '') {
      SecurityLogger.logSuspiciousActivity('Honeypot field filled', 'bot-detection');
    ...
    if (fillTime < this.MIN_FILL_TIME) {
      SecurityLogger.logSuspiciousActivity(`Form filled too quickly: ${fillTime}ms`, 'bot-detection');
