...
export const getThemeFromClass = (danceClass: DanceClass): string => {
  ...
  if (name.includes('hiphop') || name.includes('hip hop') || name.includes('commercial')) {
    return 'Hiphop';
  }
  ...
  if (name.includes('show') || name.includes('musikal') || name.includes('kompani')) {
    return 'Show';
  }
  if (name.includes('styrke') || name.includes('tøy')) {
    return 'Styrke';
  }
  if (name.includes('barnedans') || name.includes('barneballett') || 
      danceClass.age.includes('2-4') || danceClass.age.includes('5-6')) {
    return 'Toddler';
  }
  
  // Fallback basert på farge
  switch (danceClass.color?.toLowerCase()) {
    case 'orange': return 'Hiphop';
    case 'purple': return 'Jazz';
    case 'rose': return 'Ballett';
    case 'emerald': return 'Moderne';
    case 'red': return 'Show';
    case 'slate': return 'Styrke';
    case 'cyan': return 'Toddler';
    case 'fuchsia': return 'Tåspiss';
    default: return 'Hiphop';
  }
};

// Hent alle aktive schedules med kurs-info
export const fetchSchedulesWithClasses = async (): Promise<ScheduleWithClass[]> => {
  try {
    ...
    const danceClassesResponse = await listDocuments(
      ...
      [
        Query.equal('$id', danceClassIds),
        Query.limit(100)
      ]
    );

    const danceClasses = danceClassesResponse.documents as unknown as DanceClass[];
    
    // Kombiner data
    const schedulesWithClasses: ScheduleWithClass[] = schedules.map(schedule => {
      const danceClass = danceClasses.find(dc => dc.$id === schedule.dance_class_id);
...
export const fetchSchedulesByDay = async (day: string): Promise<ScheduleWithClass[]> => {
...
export const fetchSchedulesByTheme = async (theme: string): Promise<ScheduleWithClass[]> => {
  const allSchedules = await fetchSchedulesWithClasses();
  return allSchedules.filter(schedule => 
    getThemeFromClass(schedule.dance_class) === theme
  );
};
