...
export default function ExpoSliderCarousel({
  title = "Våre kurs",
  ...
  const getCourseColor = (colorName: string) => {
    ...
    const key = colorName.toLowerCase();
    return colorMap[key] || '#9333ea'; // fallback to purple
  };
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <Swiper
            ...
            onSlideChange={() => {
              // Force re-render when slide changes to fix modal issue
            }}
          >
    ...
    <SwiperSlide key={course.$id}>
      <div className="group relative">
        <div className="relative h-[400px] md:h-[450px] rounded-2xl overflow-hidden shadow-2xl transform transition-all duration-700">
          ...
          <div
            className="absolute inset-0 bg-cover bg-center transition-all duration-700 sepia"
            style={{ backgroundImage: `url(${course.image})` }}
          />
          
          {/* Glass morphism section at bottom - only visible on active slide */}
          <div className="absolute inset-x-0 bottom-0 h-40 active-slide-content opacity-0 transition-opacity duration-500">
            {/* Colored gradient background that goes to edges */}
            <div 
              className="absolute inset-0 rounded-b-2xl"
              style={{
                background: `linear-gradient(to top, ${getCourseColor(course.color)}80, ${getCourseColor(course.color)}40, transparent)`
              }}
            />
            
            {/* Glass morphism overlay that covers full width */}
            <div className="absolute inset-0 bg-black/5 backdrop-blur-md rounded-b-2xl" />
            
            {/* Content container with padding */}
            <div className="relative h-full flex flex-col justify-end p-6 md:p-8">
              <div className="bg-white/15 backdrop-blur-sm border border-white/30 rounded-xl p-4 md:p-5">
                ...
                <div className="flex gap-3">
                  <Button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      // Debugging - sjekk om event trigges
                    ...
                    className="bg-white/90 hover:bg-white text-gray-900 font-montserrat font-medium cursor-pointer relative z-10"
