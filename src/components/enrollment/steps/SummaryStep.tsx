...
export default function SummaryStep() {
  const {
    ...
    currentPricing,
    isFormValid,
    setTermsAccepted, // ✅ må finnes i context
  } = useEnrollment()
  ...
  const handleNext = () => {
    // Blokker videre hvis betingelser ikke er bekreftet
    if (!termsAccepted) {
      setShowTermsError(true)

      // Scroll + shake (Claude style)
  ...
  // Når isFormValid senere inkluderer terms, holder det å bruke isFormValid.
  // Her beholder vi eksplisitt sjekk for tydelighet:
  ...
  return (
    <div className="px-2 py-4 md:p-12">
      ...
      <div className="space-y-6">
        ...
        {!isFormValid && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
            <h4 className="font-montserrat font-semibold text-red-800 dark:text-red-200 mb-2">Noe mangler i påmeldingen:</h4>
            <ul className="text-red-600 dark:text-red-300 font-montserrat text-sm space-y-1">
              {(!state.enrollmentData.student.firstName ||
                ...
                !state.enrollmentData.student.birthDate) && <li>• Elevens informasjon er ikke fullstendig</li>}
              {(!state.enrollmentData.guardian.name ||
                !state.enrollmentData.guardian.email ||
                !state.enrollmentData.guardian.phone) && <li>• Kontaktpersonens informasjon er ikke fullstendig</li>}
              {state.enrollmentData.selectedCourses.length === 0 && <li>• Ingen kurs er valgt</li>}
              {!currentPricing && <li>• Prisberegning mangler</li>}
              {state.enrollmentData.hasSiblings &&
                state.enrollmentData.siblings.some((s) => !s.firstName.trim() || !s.lastName.trim()) && (
      ...
      {/* ✅ Terms gate (Claude UX, men controlled by context) */}
      <motion.div
        ref={(node) => {
      ...
      >
        <Button onClick={goToPreviousStep} variant="outline" className="px-6 py-3 rounded-full font-montserrat font-medium">
