...
// ── Hardkodede sommerkurs-priser (5 uker) ──────────────────
const SOMMERKURS_PRISER = [
  { navn: "1 klasse 45 min (5 uker)",              pris: "440 kr" },
  { navn: "1 klasse 60 min (5 uker)",              pris: "520 kr" },
  { navn: "2 klasser 60 min (5 uker)",             pris: "1 000 kr" },
  { navn: "3 klasser 60 min (5 uker)",             pris: "1 450 kr" },
  { navn: "4 klasser 60 min (5 uker)",             pris: "1 800 kr" },
  { navn: "5 eller flere klasser 60 min (5 uker)", pris: "2 100 kr" },
];
...
function PricingModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-lg max-h-[95vh] overflow-y-auto bg-white dark:bg-surface-dark mx-auto my-2 sm:my-8 p-4 sm:p-6">
        ...
        <div className="space-y-4 sm:space-y-6 mt-4">
          ...
          <div className="bg-gradient-to-br from-brand-50/80 to-surface-muted dark:from-brand-900/10 dark:to-surface-dark-muted p-4 sm:p-6 rounded-xl border border-brand-100/50 dark:border-brand-700/30 relative overflow-hidden">
                ...
                <div className="space-y-3">
                  {pkg.discount_amount && pkg.discount_amount > 0 ? (
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600 dark:text-gray-400 font-montserrat text-sm">Ordinær pris:</span>
                        <span className="line-through text-gray-500 font-montserrat text-sm">{formatPrice(pkg.price)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-green-600 dark:text-green-400 font-montserrat font-medium text-sm">Du sparer:</span>
                        <span className="text-green-600 dark:text-green-400 font-montserrat font-medium text-sm">{savings}</span>
                      </div>
                      <div className="border-t border-brand-200/50 dark:border-brand-700/50 pt-2">
                        <div className="flex justify-between items-center">
                          <span className="text-gray-900 dark:text-white font-montserrat font-semibold">Din pris:</span>
                          <span className="text-gray-900 dark:text-white font-montserrat font-semibold text-lg">{formatPrice(finalPrice || 0)}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-900 dark:text-white font-montserrat font-semibold">Pris:</span>
                      <span className="text-gray-900 dark:text-white font-montserrat font-semibold text-lg">{formatPrice(pkg.price)}</span>
                    </div>
                  )}
                  {pkg.discount_text && pkg.discount_text.trim() !== "" && (
                    <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3 mt-3">
                      <p className="text-sm text-green-700 dark:text-green-300 font-montserrat font-medium">{pkg.discount_text}</p>
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-gray-600 dark:text-gray-400 font-montserrat">Kontakt oss for prisinformasjon</p>
              )}
            </div>
          </div>
        </div>
        ...
        <DialogFooter className="mt-4 sm:mt-6">
          <Button
            className="w-full font-medium bg-transparent border-2 border-brand-300 text-brand-600 hover:bg-brand-50 hover:text-brand-600 dark:border-brand-700 dark:text-brand-400 dark:hover:bg-brand-900/30 dark:hover:text-brand-400 text-sm sm:text-base"
...
export default function PricingPage() {
  ...
  const fetchPricingFromAppwrite = async () => {
    ...
    setError(null);
    try {
      const response = await listDocuments(DATABASE_ID, COLLECTIONS.PRICING_PACKAGES, [
        Query.equal("is_active", true),
        Query.orderAsc("order"),
        Query.limit(50),
      ]);
      setPricingPackages(response.documents as unknown as PricingPackage[]);
    } catch (err) {
      ...
      <div className="min-h-screen bg-white dark:bg-surface-dark flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6">
            <h2 className="font-bebas text-bebas-lg text-red-800 dark:text-red-200 mb-2">Kunne ikke laste priser</h2>
            <p className="text-red-600 dark:text-red-300 font-montserrat mb-4">{error}</p>
            <Button onClick={fetchPricingFromAppwrite} className="font-semibold bg-red-600 hover:bg-red-700 text-white">Prøv igjen</Button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white dark:bg-surface-dark">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          >
            <motion.h1
              ...
              className="text-xl font-medium text-brand-600 dark:text-brand-400 uppercase tracking-wider mb-3"
            >
              Priser
            </motion.h1>
            <motion.h2
      ...
      {/* Prisliste */}
      <section className="py-16 bg-gradient-to-br from-brand-50/80 to-surface-muted dark:from-brand-900/10 dark:to-surface-dark-muted">
        <div className="container mx-auto px-4 md:px-6">
          <div className="max-w-4xl mx-auto">
            <div className="bg-white/80 dark:bg-surface-dark/80 backdrop-blur-sm rounded-2xl shadow-brand-lg border border-brand-100/50 dark:border-brand-700/30 overflow-hidden">

              {/* Header */}
              <div className="bg-gradient-to-r from-brand-500/5 to-magenta-500/5 dark:from-brand-400/10 dark:to-magenta-400/10 px-6 py-4 border-b border-brand-100/30 dark:border-brand-700/30">
                <h3 className="font-bebas text-bebas-lg text-gray-900 dark:text-white">Kurspriser</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 font-montserrat mt-1">
                  Klikk på en prispakke for mer detaljer
                </p>
              </div>

              {/* ── Semesterkurs fra Appwrite (med modal) ── */}
              <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
                {pricingPackages.map((pkg, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="absolute inset-0 bg-gradient-to-r from-brand-50/0 to-brand-50/50 dark:from-brand-900/0 dark:to-brand-900/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      <button
                        onClick={() => handleOpenModal(pkg)}
                        className="relative w-full flex items-center justify-between py-4 px-6 transition-all duration-200 group-hover:translate-x-1 text-left cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-surface-dark rounded-lg"
                      >
                        <div className="flex items-center gap-4 min-w-0 flex-1">
                          <div className="rounded-full shadow-sm bg-brand-50 dark:bg-brand-900/30 group-hover:bg-brand-100 dark:group-hover:bg-brand-900/50 text-brand-600 dark:text-brand-400 transition-all duration-200 group-hover:shadow-brand-glow group-hover:scale-105 flex-shrink-0 flex items-center justify-center p-1.5">
                            <Info className="h-3.5 w-3.5" />
                          </div>
                          <h4 className="font-montserrat font-semibold text-gray-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors duration-200 truncate">
                            {pkg.name}
                          </h4>
                        </div>
                        <div className="flex items-center gap-3 flex-shrink-0">
                          <div className="font-montserrat font-medium text-sm text-gray-900 dark:text-white text-right">
                            {pkg.price && pkg.price > 0
                              ? formatPrice(finalPrice)
                              : <span className="text-gray-500 dark:text-gray-400">Kontakt oss</span>}
                          </div>
                          <ArrowRight className="h-4 w-4 text-gray-300 dark:text-gray-600 group-hover:text-brand-500 dark:group-hover:text-brand-400 transition-all duration-200 group-hover:translate-x-1 flex-shrink-0" />
                        </div>
                      </button>
                    </motion.div>
                  );
              ...
              {/* ── Separator: skille mellom semester og sommerkurs ── */}
              <div className="relative">
                {/* Tynn linje øverst */}
                <div className="h-px bg-brand-100/60 dark:bg-brand-700/30" />

                {/* Sommerkurs-label som et naturlig skille */}
                <Link
  to="/sommerkurs"
  className="px-6 py-4 bg-gradient-to-r from-brand-500/6 via-magenta-500/4 to-brand-500/6 dark:from-brand-400/8 dark:via-magenta-400/6 dark:to-brand-400/8 flex items-center justify-between group transition-colors hover:from-brand-500/10 hover:to-magenta-500/8"
  aria-label="Les mer om sommerkurs 2026"
>
  <div className="flex items-center gap-3">
    <div className="h-5 w-0.5 rounded-full bg-brand-gradient" />
    <span className="font-bebas text-bebas-base text-gray-900 dark:text-white uppercase tracking-wide">
      Sommerkurs 2026
    </span>
    <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-montserrat font-medium bg-brand-100 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300">
      5 uker
    </span>
  </div>
  <ArrowRight className="h-4 w-4 text-brand-400 dark:text-brand-500 group-hover:text-brand-600 dark:group-hover:text-brand-300 group-hover:translate-x-1 transition-all duration-200 flex-shrink-0" />
</Link>
              ...
              {/* ── Hardkodede sommerkurs-priser (ingen modal) ── */}
              <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
                {SOMMERKURS_PRISER.map((pkg, index) => (
                  <motion.div
                    key={pkg.navn}
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.4, delay: (pricingPackages.length + index) * 0.06 }}
                    className="flex items-center justify-between py-4 px-6"
                  >
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                      {/* Liten prikk istedenfor Info-ikon */}
                      <div className="h-1.5 w-1.5 rounded-full bg-brand-400 dark:bg-brand-500 flex-shrink-0 ml-0.5" />
                      <span className="font-montserrat font-semibold text-gray-900 dark:text-white text-sm md:text-base">
                        {pkg.navn}
                      </span>
                    </div>
                    <span className="font-montserrat font-medium text-sm text-gray-900 dark:text-white flex-shrink-0 ml-4">
                      {pkg.pris}
                    </span>
                  </motion.div>
                ))}
              ...
              {/* Footer */}
              <div className="bg-gray-50/50 dark:bg-gray-800/30 px-6 py-4 border-t border-gray-100/50 dark:border-gray-700/30">
      ...
      <PricingModal pkg={selectedPackage} isOpen={isModalOpen} onOpenChange={handleCloseModal} />
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Link to="/kontakt" className="w-full sm:w-auto">
                <Button
                  variant="outline"
                  className="font-semibold rounded-full bg-white/80 border-brand-300 text-brand-700 hover:bg-brand-50 hover:text-brand-800 dark:bg-transparent dark:border-brand-700 dark:text-brand-400 dark:hover:bg-brand-900/30 dark:hover:text-brand-300"
                >
                  Kontakt oss
