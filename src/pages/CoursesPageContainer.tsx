...
const CoursesPageContainer: React.FC = () => {
  ...
  const handleAddToCart = async (studentData: StudentData) => {
    try {
      ...
      console.log("📊 Updated cart summary:", {
        ...
        familyDiscountApplied: cartSummary.items.some(
  ...
  const handleToggleFamilyDiscount = (itemId: string) => {
    try {
      console.log(
        "🔄 CoursesPageContainer: Toggling family discount for item:",
        itemId,
      );
      ...
      if (item) {
        ...
        console.log(
          "   - Current family discount status:",
  ...
  const handleDetectFamily = (firstName: string, lastName: string) => {
    try {
      console.log(
        "🔍 CoursesPageContainer: Detecting family for:",
        firstName,
        lastName,
      );
  ...
  const handleCheckout = () => {
    try {
      ...
      const cartItems = getCartItemsWithHelpers();
      const familyGroups = cartItems.reduce(
        (groups, item) => {
  ...
  if (!packages || packages.length === 0) {
    console.warn(
      "⚠️ No pricing packages found. Ensure your database is populated.",
    );
  }
  ...
  if (!danceClasses || danceClasses.length === 0) {
    console.warn(
      "⚠️ No dance classes found. Ensure your database is populated.",
    );
  }
