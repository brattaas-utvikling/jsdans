...
export const validateContactForm = async (data: Record<string, unknown>): Promise<{
  isValid: boolean;
...
export const validateField = async (
  ...
  try {
    ...
    const testData = { [fieldName]: fieldName === 'phone' && !value ? undefined : value };
...
export const performSecurityChecks = (data: ContactFormData): {
  passed: boolean;
  reason?: string;
} => {
  ...
  const spamKeywords = [
    ...
    'free money', 'make money', 'work from home', 'bitcoin',
